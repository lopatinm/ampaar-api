...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public static function setRoleUser($request)
    {
        $response = array();
        $id = $request['id'];
        ...
        try {
            ...
            if ($user && password_verify($password, $user->password)) {
                try {
                    ...
                    throw new HttpException(403, sprintf($e));
                }                
     ...
     * @throws HttpException
     */
    public static function isLoggedUser($request)
    {
        $response = array();
        try {
            $token = $request['token'];
            $user = static::findByUserToken($token);
            if ($user) {
                try {
                    $user->token = Yii::$app->security->generateRandomString(64);
                    $user->save();
